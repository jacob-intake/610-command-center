  ...
  return (
    <div style={{ background: colors.bg, border: `1px solid ${colors.border}`, borderRadius:"6px", overflow:"hidden", display:"flex", flexDirection:"column" }}>
      <div style={{ padding:"10px 14px", borderBottom:`1px solid ${colors.border}`, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <span style={{ fontSize:"10px", color: colors.label, fontFamily:"'Helvetica Neue',Arial,sans-serif", textTransform:"uppercase", letterSpacing:"1.2px", fontWeight:"600" }}>
          {caption.type}
        </span>
      ...
      <div style={{ position:"relative", width:"100%", paddingBottom:"100%", background:"#0a0a0a", overflow:"hidden" }}>
        {imageLoading && (
          <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"12px" }}>
            ...
            <span style={{ fontSize:"11px", color:"#333", fontFamily:"monospace" }}>Generating image...</span>
          </div>
        )}
      ...
      <div style={{ padding:"14px", flex:1 }}>
        <p style={{ fontSize:"13px", color:"#ccc", fontFamily:"'Helvetica Neue',Arial,sans-serif", lineHeight:"1.7", margin:0 }}>
          {caption.text}
        </p>
      </div>

      <div style={{ padding:"10px 14px", borderTop:`1px solid ${colors.border}`, display:"flex", gap:"6px" }}>
        <button onClick={handleCopy} style={btnStyle("#161616","#2a2a2a","#888")}>
          {copied ? "Copied" : "Copy"}
        </button>
        <button onClick={handleDownload} style={btnStyle("#161616","#2a2a2a","#888")}>
          Download Text
        </button>
        {imageUrl && (
          <button onClick={handleDownloadImage} style={btnStyle("#161616","#2a2a2a","#888")}>
            Download Image
          </button>
        )}
        <button onClick={() => onSchedule(caption)} style={{ ...btnStyle("#fff","#fff","#000"), marginLeft:"auto", fontWeight:"700" }}>
          Schedule
        </button>
      </div>
    </div>
...
function BlogCard({ blog, month, primaryTopic }) {
  ...
  function handleDownload() {
    const content = `610 Marketing & PR\n${month} - Blog Outline ${blog.number}\n\n${fullText}`;
    downloadText(`610-blog-outline-${blog.number}.txt`, content);
  }
  ...
  return (
    <div style={{ background:"#0d0d0d", border:"1px solid #1e1e1e", borderRadius:"6px", overflow:"hidden" }}>
      <div style={{ padding:"16px 20px", borderBottom:"1px solid #1a1a1a", display:"flex", alignItems:"center", justifyContent:"space-between", cursor:"pointer" }} onClick={() => setExpanded(!expanded)}>
        <div style={{ flex:1 }}>
          <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"6px" }}>
            <span style={{ fontSize:"10px", color:"#444", fontFamily:"monospace", border:"1px solid #222", padding:"2px 7px", borderRadius:"2px" }}>Blog {blog.number}</span>
          </div>
        ...
        <span style={{ fontSize:"18px", color:"#444", marginLeft:"16px" }}>{expanded ? "−" : "+"}</span>
      </div>

      {expanded && (
        <div style={{ padding:"20px" }}>
          <p style={{ fontSize:"13px", color:"#888", fontFamily:"'Helvetica Neue',Arial,sans-serif", lineHeight:"1.7", margin:"0 0 20px 0", paddingBottom:"16px", borderBottom:"1px solid #161616" }}>
            {blog.summary}
          </p>
      ...
      <div style={{ padding:"10px 16px", borderTop:"1px solid #161616", display:"flex", gap:"6px" }}>
        <button onClick={handleCopy} style={btnStyle("#161616","#2a2a2a","#888")}>{copied ? "Copied" : "Copy Outline"}</button>
        <button onClick={handleDownload} style={btnStyle("#161616","#2a2a2a","#888")}>Download</button>
    ...
    <div style={{ position:"fixed", inset:0, background:"#000000cc", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000 }}>
      <div style={{ background:"#111", border:"1px solid #2a2a2a", borderRadius:"6px", padding:"32px", maxWidth:"480px", width:"90%" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"20px" }}>
          ...
          <button onClick={onClose} style={{ background:"none", border:"none", color:"#555", fontSize:"20px", cursor:"pointer" }}>x</button>
        </div>
        ...
        <div style={{ background:"#0d0d0d", border:"1px solid #1a3a6b", borderRadius:"4px", padding:"16px", marginBottom:"20px" }}>
          <p style={{ fontSize:"12px", color:"#4a90d9", fontFamily:"'Helvetica Neue',Arial,sans-serif", margin:"0 0 6px 0", fontWeight:"600", textTransform:"uppercase", letterSpacing:"1px" }}>Buffer Integration</p>
          <p style={{ fontSize:"12px", color:"#555", fontFamily:"'Helvetica Neue',Arial,sans-serif", lineHeight:"1.6", margin:0 }}>Direct Buffer scheduling is coming in the next update. For now, copy this caption and paste it directly into Buffer to schedule your post.</p>
        </div>
        <div style={{ display:"flex", gap:"8px" }}>
          <button onClick={() => { navigator.clipboard.writeText(caption.text); }} style={{ ...btnStyle("#fff","#fff","#000"), flex:1, padding:"10px", fontWeight:"700", fontSize:"12px" }}>Copy Caption</button>
          <button onClick={onClose} style={{ ...btnStyle("#161616","#2a2a2a","#666"), padding:"10px 20px" }}>Close</button>
        </div>
      </div>
    </div>
  );
...
export default function CommandCenter() {
  ...
  async function handleGenerate() {
    ...
    setResult(null);
    setImages({});
    setLoadingStep("Writing 25 captions and 4 blog outlines...");
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) { setError(data.error || "Generation failed."); setLoading(false); return; }

      setResult(data);
      setActiveTab("captions");
      setLoading(false);
      setLoadingImages(true);
      setLoadingStep("Generating images for all 25 captions...");

      const imageRes = await fetch("/api/images", {
        method:"POST",
        headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ captions: data.captions, primaryTopic, secondaryTopic }),
      });
      const imageData = await imageRes.json();

      if (imageData.success) {
        const imageMap = {};
        imageData.images.forEach(img => { if (img.imageUrl) imageMap[img.number] = img.imageUrl; });
        setImages(imageMap);
      }
    } catch (err) {
  ...
  if (!authenticated) {
    return (
      <>
        <style>{`* { box-sizing:border-box; margin:0; padding:0; } body { background:#000; } @keyframes fadeIn { from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)} } .auth-card{animation:fadeIn 0.4s ease forwards} input::placeholder{color:#444} input:focus{border-color:#555!important;outline:none}`}</style>
        <div style={{ minHeight:"100vh", background:"#000", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Helvetica Neue',Arial,sans-serif" }}>
          <div className="auth-card" style={{ background:"#0d0d0d", border:"1px solid #1e1e1e", borderRadius:"6px", padding:"52px 44px", width:"100%", maxWidth:"360px", display:"flex", flexDirection:"column", alignItems:"center", gap:"14px" }}>
  ...
  const isGenerating = loading || loadingImages;
      ...
      <div style={{ minHeight:"100vh", background:"#0a0a0a", color:"#f0f0f0", fontFamily:"'Helvetica Neue',Arial,sans-serif", display:"flex", flexDirection:"column" }}>
        ...
        <main style={{ maxWidth:"1600px", margin:"0 auto", padding:"40px 40px 60px", flex:1, width:"100%" }}>
          <div style={{ marginBottom:"32px" }}>
            ...
            <p style={{ fontSize:"13px", color:"#444" }}>Generate 25 social captions with images and 4 blog outlines in 610 voice</p>
          </div>
              ...
              <button className="gen-btn" onClick={handleGenerate} disabled={isGenerating || !primaryTopic.trim()} style={{ width:"100%", padding:"13px", background:(isGenerating||!primaryTopic.trim())?"#161616":"#fff", color:(isGenerating||!primaryTopic.trim())?"#2a2a2a":"#000", border:"none", borderRadius:"3px", fontSize:"13px", fontWeight:"700", cursor:(isGenerating||!primaryTopic.trim())?"not-allowed":"pointer", letterSpacing:"1px", textTransform:"uppercase", transition:"background 0.15s" }}>
                {isGenerating ? (
                  <span style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:"10px" }}>
                    ...
                    {loading ? "Writing content..." : "Generating images..."}
                  </span>
                ) : "Generate This Month's Content"}
              </button>

              {isGenerating && (
                <p style={{ fontSize:"12px", color:"#444", textAlign:"center", marginTop:"-6px", animation:"pulse 1.5s ease infinite" }}>{loadingStep}</p>
              )}
              ...
              {result && (
                <>
                  <div style={{ display:"flex", borderBottom:"1px solid #1a1a1a" }}>
                    ...
                    {loadingImages && (
                      <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:"8px", paddingRight:"4px" }}>
                        <span style={{ width:"10px", height:"10px", border:"2px solid #33333388", borderTop:"2px solid #4a90d9", borderRadius:"50%", animation:"spin 0.8s linear infinite", display:"inline-block" }} />
